...
3. Run **`terraform apply`** on **`k8s_platform`** (or merge a change under `infra/aws/` so GitHub Actions applies). Terraform installs **[ExternalDNS](https://github.com/kubernetes-sigs/external-dns)** with **IRSA**; it watches **Ingress** resources and creates **alias** records (and ownership TXT records) in that zone for hostnames such as **`api.k8s.michaelj43.dev`** and the zone apex **`k8s.michaelj43.dev`** (portal Ingress in `deploy/base/portal`) once the AWS Load Balancer Controller has published the ALB address on each Ingress.

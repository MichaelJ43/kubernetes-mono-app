...
10. **Terraform apply: Argo CD Helm `context deadline exceeded`** — The install uses **`helm upgrade --install … --wait`**. On small EKS nodes or cold pulls, Argo pods may take longer than the previous 15-minute wait. Workflows use a longer Helm timeout; if it still fails, inspect `kubectl -n argocd get pods` from a working kubeconfig, then re-run **Terraform apply** (or **Argo CD bootstrap**).

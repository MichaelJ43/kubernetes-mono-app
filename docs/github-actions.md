...
10. **Terraform apply: Argo CD Helm `context deadline exceeded` or `pre-upgrade hooks failed`** — Chart hooks (not all pods) can hit the Helm `--wait` deadline. The usual stuck hook is **`redis-secret-init`** from `redisSecretInit` (pre-install/pre-upgrade). Workflows delete incomplete `*redis*secret*init*` **Jobs** in `argocd` before upgrading, pin a chart **`--version`**, and print **`kubectl` / `helm status`** diagnostics on failure. If it still fails, open the workflow log for **Argo CD Helm diagnostics** and check the listed Job/Pod events (often `ImagePullBackOff` or scheduling on a full node).

...
10. **Terraform apply: Argo CD Helm `context deadline exceeded` or `pre-upgrade hooks failed`** — The `argo-cd` chart's **`redis-secret-init`** Helm hook repeatedly timed out in this cluster. Workflows now create the **`argocd-redis`** Secret before Helm runs and set **`redisSecretInit.enabled=false`**, bypassing that hook. The Argo values also disable notifications and scale ApplicationSet to zero because this repo uses plain Applications. On failure, the workflow prints pods, pod descriptions, events, and **`helm status`** so the log names the real blocker (often `ImagePullBackOff` or scheduling on a full node).

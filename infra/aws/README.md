...
| **deploy_orchestrator** | `infra/aws/deploy_orchestrator` | Source S3 bucket for release bundles, Lambda + HTTP API (**`POST /deploy`**, **`/swap`**, **`/teardown`**), DynamoDB job status, EKS access for Lambda. Reads **`foundation`** and **`parked_site`** remote state. |

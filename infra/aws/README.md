...
3. From `infra/aws/foundation`:
   ...
   terraform apply
   ```
   Optional: set **`acm_certificate_domain`** in `terraform.tfvars` (same name as in ACM, e.g. `*.k8s.michaelj43.dev`) so **`terraform output acm_certificate_arn`** is populated; then run **`../../scripts/render-ingress-acm-patch.sh`** from repo root and commit the updated `deploy/overlays/aws-prod/ingress-acm-patch.yaml` for Argo (see [`../docs/aws-domain-tls.md`](../docs/aws-domain-tls.md)).

  ...
  eks_access_entries = merge(
    {
      ...
      github_bootstrap = {
        principal_arn = data.aws_iam_role.github_bootstrap.arn
        policy_associations = {
          admin = {
            policy_arn = "arn:aws:eks::aws:cluster-access-policy/AmazonEKSClusterAdminPolicy"
            access_scope = {
    ...
    var.github_actions_deploy_role_arn != null && var.github_actions_deploy_role_arn != data.aws_iam_role.github_terraform.arn ? {
